SELECT
  ...
  'SpeciesURL'
UNION ALL
    (SELECT '5.0.1 GSD duplicates ACC-ACC species (same authors)',
       '',
...
FROM GM_GSD.GM_AcceptedSpecies acc
       INNER JOIN (SELECT concat_ws("_", tb1.GM_ACC_Genus, tb1.GM_ACC_SpeciesEpithet,
                                    tb1.GM_ACC_AuthorString) AS DupString
                   ...
                   GROUP BY tb1.GM_ACC_Genus, tb1.GM_ACC_SpeciesEpithet, tb1.GM_ACC_AuthorString
                   HAVING count(*) > 1) dups
         ON dups.DupString = concat_ws("_", acc.GM_ACC_Genus, acc.GM_ACC_SpeciesEpithet, acc.GM_ACC_AuthorString)
