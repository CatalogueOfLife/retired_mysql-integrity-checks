SELECT 'Integrity check',
       ...
       'SpeciesURL'
UNION ALL
    (SELECT '5.1.0 GSD duplicates ACC-SYN species (different parent, different authors)',
            '',
            SynID,
            ParentID,
            GM_ACC_Genus,
     ...
     FROM (SELECT NULL AS SynID,
                  ...
                  GM_ACC_Sp2000NameStatus,
                  GM_ACC_SpeciesURL
           FROM (SELECT concat_ws("_", acc.GM_ACC_Genus, acc.GM_ACC_SubGenusName,
                                  acc.GM_ACC_SpeciesEpithet) AS DupString
                 ...
                 GROUP BY acc.GM_ACC_Genus, acc.GM_ACC_SubGenusName, acc.GM_ACC_SpeciesEpithet) dups
           ...
           SELECT GM_SYN_ID                  AS SynID,
                  ...
                  ''
           FROM (SELECT concat_ws("_", syn.GM_SYN_Genus, syn.GM_SYN_SubGenusName,
                                  syn.GM_SYN_SpeciesEpithet) AS DupString
                 FROM GM_GSD.GM_AcceptedSpecies acc
                        INNER JOIN GM_GSD.GM_Synonyms syn ON acc.GM_ACC_Genus = syn.GM_SYN_Genus AND
                                                             acc.GM_ACC_SpeciesEpithet = syn.GM_SYN_SpeciesEpithet AND
                                                             (((syn.GM_SYN_SubGenusName IS NULL OR syn.GM_SYN_SubGenusName = '')
