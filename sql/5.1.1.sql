SELECT 'Integrity check',
       ...
       'SpeciesURL'
UNION ALL
    (SELECT '5.1.1 GSD duplicates ACC-SYN species (different parent, same authors)',
            '',
            SynID,
            ParentID,
            GM_ACC_Genus,
...
FROM (SELECT NULL AS SynID,
      ...
      FROM (SELECT concat_ws("_", acc.GM_ACC_Genus, acc.GM_ACC_SubGenusName, acc.GM_ACC_SpeciesEpithet,
            ...
            GROUP BY acc.GM_ACC_Genus, acc.GM_ACC_SubGenusName, acc.GM_ACC_SpeciesEpithet, acc.GM_ACC_AuthorString) dups
