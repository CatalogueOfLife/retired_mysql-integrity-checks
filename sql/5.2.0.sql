SELECT 'Integrity check',
       ...
       'Sp2000Status'
UNION ALL
    (SELECT '5.2.0 GSD duplicates SYN-SYN species (different parent, different authors)',
            '',
            GM_SYN_ID              AS SynID,
     ...
     FROM GM_GSD.GM_Synonyms syn
            INNER JOIN (SELECT concat_ws("_", tb1.GM_SYN_Genus, tb1.GM_SYN_SubGenusName,
                                         tb1.GM_SYN_SpeciesEpithet) AS DupString
                        ...
                        HAVING count(*) > 1) dups
              ON dups.DupString = concat_ws("_", syn.GM_SYN_Genus, syn.GM_SYN_SubGenusName, syn.GM_SYN_SpeciesEpithet)
